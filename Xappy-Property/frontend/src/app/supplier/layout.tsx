...
const navItems = [
  { name: "My Jobs", href: "/supplier/jobs", icon: Briefcase },
  { name: "Availability", href: "/supplier/availability", icon: Calendar },
  { name: "Invoices", href: "/supplier/invoices", icon: FileText },
  { name: "Help & Support", href: "/supplier/help", icon: HelpCircle },
...
export default function SupplierLayout({
  ...
  const getPageTitle = () => {
    ...
    return item?.name || "Supplier Portal";
  };
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200">
            <Link href="/supplier/jobs" className="flex items-center space-x-2">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={`h-5 w-5 mr-3 ${isActive ? "text-orange-600" : "text-gray-400"}`} />
                  {item.name}
                </Link>
              );
            })}
