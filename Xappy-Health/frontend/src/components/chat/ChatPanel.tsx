...
// Modern Xappy Welcome Card Component - Professional Healthcare Colors
// Color Psychology: Blue = Trust/Calm, Teal = Refreshing/Reliable, Green = Health/Wellness
function renderXappyWelcomeCard(onQuickAction: (query: string) => void): React.ReactNode {
  const quickActions = [
    {
      ...
      icon: Search,
      title: "Find Care",
      subtitle: "Hospitals & Doctors nearby",
      query: "find hospitals in my area",
    ...
    },
    {
      ...
      icon: HeartPulse,
      title: "Symptom Check",
      subtitle: "Get guidance on symptoms",
    ...
    },
    {
      ...
      icon: ClipboardList,
      title: "Vaccinations",
      subtitle: "Programs & schedules",
      query: "show vaccination programs",
  ...
  const additionalOptions = [
    { icon: Sparkles, label: "Health Tips", query: "give me health tips", color: "text-teal-600", bg: "bg-teal-50", border: "border-teal-100" },
    { icon: Baby, label: "Pregnancy & Childcare", query: "pregnancy guidance", color: "text-cyan-600", bg: "bg-cyan-50", border: "border-cyan-100" },
    { icon: Phone, label: "Emergency Help", query: "emergency contacts", color: "text-red-600", bg: "bg-red-50", border: "border-red-100" },
    { icon: AlertTriangle, label: "Health Alerts", query: "health alerts in my area", color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-100" },
  ];

  return (
    <div className="space-y-4">
      {/* Xappy Welcome Header - Professional Healthcare Blue/Teal */}
      <div className="rounded-2xl overflow-hidden shadow-lg border border-cyan-100">
        <div className="relative bg-gradient-to-br from-cyan-600 via-teal-600 to-emerald-600 p-6">
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-white/95 text-sm leading-relaxed">
                Your trusted healthcare companion. I can help you find doctors,
                understand symptoms, track vaccinations, and guide your health journey.
              </p>
              <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20">
                ...
                <span className="text-white/90 text-xs font-medium">Health guidance only • Not medical advice</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Quick Actions Grid - Clean Healthcare Cards */}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-2.5 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">More Services</p>
        </div>
      ...
      {/* Conversation Starters - Healthcare Teal Theme */}
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-white text-slate-600 border border-teal-200 hover:border-teal-400 hover:text-teal-700 hover:bg-teal-50 transition-all shadow-sm"
            >
              {suggestion}
            </button>
          ))}
...
type ChatMode = "safety" | "healthcare-demo";
...
const DEMO_SUGGESTION_CHIPS = [
  { label: "Find Hospital/Doctor", query: "find hospitals in Colombo", icon: Search, color: "primary" },
  { label: "Check Symptoms", query: "check symptoms", icon: AlertTriangle, color: "warning" },
  { label: "Vaccination & Programs", query: "vaccination programs", icon: ClipboardList, color: "success" },
  { label: "Lab Report", query: "lab report explanation", icon: FileText, color: "purple" },
  { label: "Complaint Ticket", query: "complaint", icon: ClipboardList, color: "pink" },
];
...
export default function ChatPanel({ mode = "safety" }: { mode?: ChatMode }) {
  ...
  }, [isHealthcareDemo, messages.length]);
  ...
  const sendMessage = async (
    ...
    if (isHealthcareDemo) {
      ...
      try {
        const response = await fetch("/api/v1/healthcare/chat/send", {
          method: "POST",
  ...
  const welcomeTitle = isHealthcareDemo ? "Healthcare Information Hub" : "Xappy";
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            ...
            <h3 className="text-xl font-semibold text-slate-800 mb-2">
              {isHealthcareDemo ? "Healthcare Information Hub" : "Hi there!"}
            </h3>
            ...
            <div className="w-full max-w-md mb-4">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">
                {isHealthcareDemo ? "Demo Flows" : "Quick Report"}
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {(isHealthcareDemo ? DEMO_SUGGESTION_CHIPS : SUGGESTION_CHIPS.reports).map((chip) => {
            ...
            <div className="w-full max-w-md">
              <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">
                {isHealthcareDemo ? "Start" : "View Reports"}
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                {(isHealthcareDemo
                  ? [
          ...
          <>
            {messages.map((msg, index) => (
              <div key={msg.id}>
                ...
                {msg.role === "assistant" && msg.content === "__MAYA_WELCOME__" && isHealthcareDemo ? (
                  <div className="mb-4">
            ...
            {messages.length > 0 && !sending && messages[messages.length - 1].role === "assistant" && !messages[messages.length - 1].draftState && !messages[messages.length - 1].submissionResult && messages[messages.length - 1].content !== "__MAYA_WELCOME__" && (
              <div className="ml-10 mt-2">
                <div className="flex flex-wrap gap-2">
                  {(isHealthcareDemo ? DEMO_SUGGESTION_CHIPS.slice(0, 3) : SUGGESTION_CHIPS.reports.slice(0, 3)).map((chip) => {
                    const Icon = chip.icon;
                    return (
                      <button
                        key={chip.query}
